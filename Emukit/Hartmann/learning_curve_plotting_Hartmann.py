...
def euclidean_distance_ackley(x):
...
def scatter_25_and_75_percentile_track_X(Output,n_lhs_samples,total,bs,iterations,ground_y_max,beta,std_noise,filename,ker_variance_filename):
    ...
    for iteration_num in range(len(output[0])):
        data = [euclidean_distance_hartmann(run[iteration_num][1],iteration_num) for run in output] 
        data=np.array(data)  
        x=iteration_num;length=len(data)          
    ...
    label='25% Percentile'         
    for iteration_num in range(len(output[0])): 
        dataY25=euclidean_distance_hartmann(output[index25][iteration_num][1],iteration_num)      
    ...
    label='75% Percentile'     
    for iteration_num in range(len(output[0])): 
        dataY75=euclidean_distance_hartmann(output[index75][iteration_num][1],iteration_num)       
    ...
    data50=[];ACR_X=0
    for iteration_num in range(len(output[0])):  
        dataY=euclidean_distance_hartmann(output[indexY][iteration_num][1],iteration_num)   
        data50.append(dataY) 
    ...
    # plt.legend(loc='upper left', bbox_to_anchor=(1.0, 1.0),ncol=3)
    ...
    for run in output:
        data = euclidean_distance_hartmann(run[len(output[0])-1][1],iteration_num=49) 
        sum_IR_X+=data
        data = np.abs(ground_y_max-run[len(output[0])-1][0]) 
        #print(data,run[len(output[0])-1][0])
        sum_IR_Y+=data

    print(sum_IR_Y,sum_IR_X)
    ...
    for run in output:
        ...
        for i in range(total_iteration):
            data = euclidean_distance_hartmann(run[i][1],iteration_num=i) 
            sum_X+=data
...
def kernel_parameter_lengthscale_plotting(Output,n_lhs_samples,total,bs,iterations,ground_y_max,beta,std_noise,index,percentile25_X_index,percentile75_X_index,filename):
...
def kernel_variance_plotting(Output,n_lhs_samples,total,bs,iterations,ground_y_max,beta,std_noise,index,percentile25_X_index,percentile75_X_index,filename):
...
def gaussian_variance_plotting(Output,n_lhs_samples,total,bs,iterations,ground_y_max,beta,std_noise,index,percentile25_X_index,percentile75_X_index,filename):
    ...
    text = f"$\:\ Gaussian \ Variance \ at\ End\ =\ {last_data:0.6f}$"
    ...
    plt.tick_params(direction='in')      
    plt.xlabel("Iteration")
    plt.ylabel(r'Gaussian Variance')
    plt.xlim(-1,51)
    plt.ylim(-0.2,8)
    yticks=[0,2,4,6,8]
    plt.yticks(yticks)
...
def plotting(Output,n_lhs_samples,total,bs,iterations,ground_y_max,beta,std_noise,filename,ker_variance_filename,ker_lengthscale_filename):
    ...
    # # X tracking portion, plottting X and y learning curve
    index50_75=[]
    ...
    ##Lengthscale evolution curve plotting
    ...
    ker_lengthscale=kernel_parameter_lengthscale_plotting(Output,n_lhs_samples,total,bs,iterations,ground_y_max,beta,std_noise,index_corresponded_Y,percentile25_X_index,percentile75_X_index,file_lengthscale)

    ## Kernel amplitude and GNV plotting
    directory_path=os.getcwd()
    pattern = ker_variance_filename
    matching_files = [filename for filename in os.listdir(directory_path) if filename.startswith(pattern)]
    file_variance=matching_files[0]
    ker_variance=kernel_variance_plotting(Output,n_lhs_samples,total,bs,iterations,ground_y_max,beta,std_noise,index_corresponded_Y,percentile25_X_index,percentile75_X_index,file_variance)
    gauss_variance=gaussian_variance_plotting(Output,n_lhs_samples,total,bs,iterations,ground_y_max,beta,std_noise,index_corresponded_Y,percentile25_X_index,percentile75_X_index,file_variance)
