...
def parity_plot(X,train_x,model,Y_mean,Y_std,LHS_no,beta):
    
    Y_truth=TestFunction.hartmann(X);Y_truth=np.array(Y_truth).reshape(len(Y_truth),1)  
    ...
    plot_filename='parity_plot_LHS_no'+str(LHS_no)+'_beta'+str(beta)+'.svg'
...
def create_subplot(subplot_num, subplot_row,subplot_column, x1, x2, M, X,Xmax1,Xmax2, Xtrue1,Xtrue2,X1true1,X1true2,ymax,LHS_No,beta,index1,index2):
    
    if subplot_column!=1 or subplot_row!=1:
        plt.subplot(subplot_row, subplot_column, subplot_num)
    else:
        fig1 = plt.figure(1)
        fig1.set_size_inches(2.5, 2)
    ...
    positions = [0.0, 0.6,0.85,1.0]  # First 20% transitions quickly, remaining 60% transitions slowly


    # Change colors from #101A6C to white    
    colors = [(10/255, 25/255, 120/255), (45/255, 67/255, 211/255), (125/255, 141/255, 247/255), (1, 1, 1)]  # Transition from #101A6C to white
    for pos, color in zip(positions, colors):
        #print(pos,color,'position')
        r, g, b = color
        cdict['red'].append((pos, r, r))
        cdict['green'].append((pos, g, g))
        cdict['blue'].append((pos, b, b))

    custom_cmap = LinearSegmentedColormap('CustomBlue', cdict);cmap = custom_cmap
    ...
    limit=ymax/n_contourline_bins    
    # colormap case 1..orange color
    if index1!=6:
        plt.contour(X2, X1, M, levels = np.arange(n_contourline_bins+1)*limit+c_offset, colors='#E3BB18', linestyles='dashed')
    ...
    plt.scatter(X1true1, X1true2, s=gt_marker_size, zorder=5, color='#A314CD', marker='*', label='2nd True Max')
    ...
    if subplot_column==1 and subplot_row==1:
        plt.tight_layout()
        folder_name='Heat_Maps'+'LHS_No'+str(LHS_No)+'_beta'+str(beta)
        plot_filename='X'+str(index1)+'X'+str(index2)+'.png'
        saveing_in_folder(folder_name,plot_filename)
        plt.close()

    

def predicted_model_perLHS(model_X,model,Y_mean,Y_std,ymax,LHS_No,beta):
    ...
    for i in range(2):
        if i==0:           
            ...
            create_subplot(1, subplot_row,subplot_column,x1, x2, M12, model_X,X1max, X2max, X1true, X2true,X1true1, X2true1,ymax,LHS_No,beta,index1=1,index2=2)
            create_subplot(2, subplot_row,subplot_column,x1, x3, M13, model_X,X1max, X3max, X1true, X3true,X1true1, X3true1,ymax,LHS_No,beta,index1=1,index2=3)
            create_subplot(3, subplot_row,subplot_column,x1, x4, M14, model_X,X1max, X4max, X1true, X4true,X1true1, X4true1,ymax,LHS_No,beta,index1=1,index2=4)
            create_subplot(4, subplot_row,subplot_column,x1, x5, M15, model_X,X1max, X5max, X1true, X5true,X1true1, X5true1,ymax,LHS_No,beta,index1=1,index2=5)
            create_subplot(5, subplot_row,subplot_column,x1, x6, M16, model_X,X1max, X6max, X1true, X6true,X1true1, X6true1,ymax,LHS_No,beta,index1=1,index2=6)
